...
def stage1():
    ...
    parser.add_argument("--view", help="view", action="store_true")
    parser.add_argument(
        "--text_rank_only", help="only do textrank", action="store_true"
    )
    ...
    if not args.text_rank_only:
        if table_from and table_to:
            clean_email_bodies_pipeline(
                keep_full_history=thread, table_from=table_from, table_to=table_to
            )
        elif table_from:
