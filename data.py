...
class IndexingTrainDataset(Dataset):
    ...
    def __getitem__(self, item):
        ...
        if self.remove_prompt:
            data["text"] = (
                data["text"][9:]
                if data["text"].startswith("Passage: ")
                else data["text"]
            )
            data["text"] = (
                data["text"][10:]
                if data["text"].startswith("Question: ")
                else data["text"]
            )
    ...
    def __init__(
        ...
        if ".db" in path_to_data:
            if table_name == None:
                ...
                if "body_clean_and_subject" in df.columns:
                    for mid, row in df.iterrows():
                        self.data.append(
                            (
                                mid,
                                row["body_clean_and_subject"]
                            )
                        )
                else:
                    for mid, row in df.iterrows():
                    ...
                    if "xorqa" in path_to_data:
                        ...
                        for lang in self.lang2mT5.values():
                            self.data.append(
                                (
                                    docid,
                                    f"Generate a {lang} question for this passage: {title} {passage}",
                                )
                            )
                        ...
                        raise NotImplementedError(
                            f"dataset {path_to_data} for docTquery generation is not defined."
                        )
